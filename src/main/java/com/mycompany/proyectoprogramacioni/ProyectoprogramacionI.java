    ...
    public static void main(String[] args) {
        ...
        while (opcionmenu != 6) {
            ...
            switch (opcionmenu) {
                ...
                case 2:
                    // Ver medicamento en inventario (busca en arreglos)
                    ...
                    for (int i = 0; i < numMedicamentos; i++) {
                        if (dato.trim().equalsIgnoreCase(codigos[i]) || dato.trim().equalsIgnoreCase(nombres[i])) {
                            // actualizo variables individuales para mantener el comportamiento original
                            codigo = codigos[i];
                            nombre = nombres[i];
                            precio = precios[i];
                    ...
                    for (int i = 0; i < numMedicamentos; i++) {
                        if (buscar.equalsIgnoreCase(codigos[i]) || buscar.equalsIgnoreCase(nombres[i])) {
                            ...
                            if (venta <= cantidad && venta > 0) {
                                ...
                                cantidades[i] = cantidades[i] - venta; // actualizo arreglo
